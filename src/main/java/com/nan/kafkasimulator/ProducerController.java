...
public class ProducerController implements Initializable {
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        // 延迟设置TopicManagementController的回调，确保控制器已经初始化
        javafx.application.Platform.runLater(() -> {
            TopicManagementController topicController = ControllerRegistry.getTopicManagementController();
            if (topicController != null) {
                topicController.setOnTopicsUpdated(this::updateTabs);
            }
        });
    }
            ...
            for (String topic : topicNames) {
                if (!topicControllers.containsKey(topic)) {
                    try {
                        ...
                        // 加载tab内容
                        FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/nan/kafkasimulator/fxml/producer-tab-content.fxml"));
                        // 设置控制器
